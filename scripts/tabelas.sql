...
CREATE SEQUENCE GEN_RTC_NCM_CCLASSTRIB_ID;
...
ALTER TABLE ITENS ADD RTC_CCLASSTRIB_NFCE RTC_CCLASSTRIB_C;
...
RETURNS (
    ...
    R_CST          RTC_CST_C,
    R_FONTE        VCDESCRICAO20,  -- 'OPERACOES' | 'OPERACOES_PRODUTO' |  'PRODUTO' | 'NCM_PARTICIPANTE' | 'NBS_PARTICIPANTE' | 'NCM' | 'NBS' | 'NONE'
    R_PREDIBS      RTC_PERCENTUAL,
    R_PREDCBS      RTC_PERCENTUAL
)
AS
...
BEGIN
  ...
  FOR
    SELECT CCT, FONTE
    FROM (
      SELECT t.CCLASSTRIB AS CCT, 'OPERACOES' AS FONTE, 1 AS ORD
        FROM RTC_OPERACOES t
       WHERE t.ID = :P_ID_RTC_OPERACOES AND t.CCLASSTRIB IS NOT NULL

      UNION ALL

      SELECT tp.CCLASSTRIB AS CCT, 'OPERACOES_PRODUTO' AS FONTE, 2 AS ORD
