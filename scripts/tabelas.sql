...
BEGIN
  ...
  IF ( VALIDAR_VIGENCIA(:P_DATA_EMISSAO, :V_DINI, :V_DFIM) = 1
       AND VALIDAR_MODELO(:P_MODELO_DF, :V_IND_NFE, :V_IND_NFCE, :V_IND_NFSE) = 1 ) THEN
  BEGIN
    R_OK  = 1;
...
/* =========================
   ...
   ========================= */
SET TERM ^ ;
CREATE OR ALTER PROCEDURE RTC_RESOLVE_CCLASSTRIB (
    P_ID_TOP       INTEIRO,
...
BEGIN
  ...
  FOR
    ...
    FROM (
      ...
      SELECT tp.CCLASSTRIB AS CCT, 'RTC_OPERACOES_PRODUTO' AS FONTE, 2 AS ORD
       ...
       WHERE tp.OPERACOES = :P_ID_TOP
         AND tp.PRODUTO = :P_ID_PRODUTO

      UNION ALL

      SELECT p.CCLASSTRIB AS CCT, 'PRODUTO' AS FONTE, 3 AS ORD -- VERIFICAR OS SELECTS DAS TABELAS DE ITENS (PRODUTOS) --
        FROM ITENS p
  ...
  DO
  BEGIN
    EXECUTE PROCEDURE VALIDAR_CCLASSTRIB(:V_CCT_CAND, :P_DATA_EMISSAO, :P_MODELO_DF)
